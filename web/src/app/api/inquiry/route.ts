...
import { escapeHtml, isValidLength } from '@/lib/security'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isValidLength(data.name, 100) || !isValidLength(data.phone, 50)) {
      return NextResponse.json(
        { error: 'Ungültige Eingabe (zu lang)' },
        { status: 400 }
      )
    }

    if (data.pflegegrad && !isValidLength(data.pflegegrad, 50)) {
      return NextResponse.json(
        { error: 'Ungültige Eingabe (zu lang)' },
        { status: 400 }
      )
    ...
    const safePflegegrad = data.pflegegrad ? escapeHtml(data.pflegegrad) : undefined
