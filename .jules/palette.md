...
## 2025-02-12 - React Details Element Pattern
**Learning:** Using `onClick` on `<summary>` with `e.preventDefault()` to control `<details>` state breaks native behavior and accessibility.
**Action:** Use the `open` prop on `<details>` combined with `onToggle` to sync state while preserving native browser behavior.
