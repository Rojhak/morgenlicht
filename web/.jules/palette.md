...
## 2024-05-10 - Accessibility for custom detail toggles
**Learning:** When preventing default behavior on `<summary>` elements to use custom React state for toggling visibility, native accessibility features are lost. It is crucial to manually manage `aria-expanded` and `aria-controls` to ensure screen readers can announce the state correctly.
**Action:** Always add explicit ARIA attributes when hijacking native interactive element behavior.
