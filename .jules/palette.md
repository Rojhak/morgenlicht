...
## 2025-03-04 - Form Loading and Error Micro-UX\n**Learning:** Contact form lacked explicit error message association (`aria-describedby`), invalid state (`aria-invalid`), explicit loading states (`aria-busy`, spinner), and screen-reader safe required indicators.\n**Action:** Implemented standard micro-UX enhancements to inputs and buttons. Confirmed that local visual verification with Playwright is more stable when running `pnpm start` instead of `pnpm dev`.
