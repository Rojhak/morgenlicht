...
## 2025-05-04 - Type Confusion in Sanitization Methods
**Vulnerability:** Type confusion in `sanitizeInput` and `sanitizeForSubject` where truthy non-string types (like arrays) bypassed the `!input` check, causing a `TypeError` crash when calling `.replace()`.
**Learning:** Relying solely on truthiness checks is insufficient for input validation in API routes where inputs can be manipulated to be unexpected types (like JSON arrays).
**Prevention:** Always explicitly check the type (`typeof input === "string"`) before calling string-specific methods to prevent unhandled exceptions and potential Denial of Service (DoS).
